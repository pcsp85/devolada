...
if($_POST["act"]=="send"){
	...
	$cuerpo="<html><body>".$_POST["cuerpo"]."</body></html>";
	$de=mysql_query("select * from destinatarios where id_ld = $_POST[ld]");
	while($d=mysql_fetch_array($de)){
		$no[]="\\"; $si[]="";
		foreach($d as  $c=>$v){ $no[]="*$c*"; $si[]=utf8_decode("$v"); }
		$mail = new PHPMailer();
		//$mail->IsSMTP();
		$mail->Host=$ms;
		$mail->SMTPAuth=false;
		...
		$mail->Body=str_replace($no,$si,($cuerpo));
		if(!$mail->Send()){
			$det_falla.="$d[id]-".$mail->ErrorInfo."*";
			$fallas++;
		}else $enviados++;
		$no=false; $si=false;
	}
	mysql_query("insert into reporte (id, id_ld, fecha, usr, enviados, fallas, det_falla, cuerpo, asunto) values ('','$_POST[ld]', UNIX_TIMESTAMP(), '$_SESSION[usr]', '$enviados', '$fallas', '$det_falla', '$cuerpo', '$_POST[subject]')");
	header("Location: ./");
}
 ?>
<!DOCTYPE HTML>
...
function vf(){
	var err="";
	if(document.enviar.ld.value=="") err+="- Debes selecionar una Lista de Distribución.\n";
	if(document.enviar.subject.value=="") err+="- Debes escribir el Asunto.\n";
...
<div class="container">
  <header>
    <a href="#"><img src="image/logoW3D.jpg" alt="W3D Creative Sevices - Design" width="180" height="90" id="Insert_logo" style="background: #C6D580; display:block;" /></a>
  </header>
  <div class="sidebar1">
    <nav>
      <? if($_SESSION["mailer"]=="activo"){?>
      <ul>
        <li><a href="envio.php">Enviar Correo Masivo</a></li>
...
<p><br><br><br></p>
  ...
  <article class="content">
    ...
    <section>
     ...
     <? $li=mysql_query("select id,lista from listas order by lista");
	 $hr=mysql_fetch_array(mysql_query("select fecha from reporte order by id desc limit 1"));
	 if(mysql_num_rows($li)==0) echo "<p>No existen Listas de Distribuci&oacute;n, <a href='lista.php'>da Click para agregar una Lista</a></p>";
	 //elseif($hr[0]!="" && mktime()-$hr[0]<=3600) echo "<p>No es posible realizar la operación, debido a que no ha pasado el tiempo indicado, el último envío fue realizado el ".date("d/m/Y \a \l\a\s H:i",$hr[0]-3600)."hrs.</p>";
	 else{ ?>
      <form name="enviar" method="post" enctype="multipart/form-data">
      <input type="hidden" name="act" value="send">
      <p>Lista de Distribución: 
      <select name="ld">
      <option value="">Selecciona una Lista de Distribución</option>
      <? while($l=mysql_fetch_array($li)){ ?>
      <option value="<?=$l["id"]?>"><?=$l["lista"]?></option>
      <? } ?>
      </select>
      </p>
      <p>Asunto:<input type="text" name="subject" size="50"></p>
      <p>De: <input type="text" name="fromname" title="Nombre de quien envía"> &lt;<input type="text" name="from" title="Correo de quien envía">&gt;</p>
      <p>Mensaje:<br>
      <textarea name="cuerpo"></textarea>
      </p>
      
      <center><input type="submit" value="Enviar" onClick="return vf();"></center>
      </form>
      <script>
